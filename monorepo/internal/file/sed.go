...
func Sed(inputFile string, operations []LineOperation) error {
	...
	inputFileBackup := inputFile + ".bak"
	err = Copy(inputFile, inputFileBackup)
	if err != nil {
		return err
	}

	outputFile := inputFile
	inputFile = inputFileBackup

	err = processFiles(inputFile, outputFile, operations)
	if err != nil {
		return err
	}
	...
	err = os.Remove(inputFileBackup)
	if err != nil {
		return fmt.Errorf("unable to remove backup file '%s': %w", inputFileBackup, err)
	}
...
func processFiles(inputFile, outputFile string, operations []LineOperation) error {
